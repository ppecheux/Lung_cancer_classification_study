...
for(k in (1:K)){
  ...
  #Logreg
  logreg.bc_data <- multinom(as.factor(classes)~. , data = bc_data.train[folds!=k,c(-1,-12)])#fit
  pred.bc_data.logreg<-predict(logreg.bc_data,newdata=bc_data.train[folds==k,c(-1,-12)],type="class")#pred
  CV[k,4]<-sum(pred.bc_data.logreg!=bc_data.train$classes[folds==k])
}
CV
```
```{r}
